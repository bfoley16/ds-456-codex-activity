...
- What did Codex assume or gloss over?
  It assumed that we were determined to use the dataset, as it provided code to load and explore the dataset into VScode. It assumed that we were only interested in data from Open Data Minneapolis. 
- Did it make any mistakes or questionable choices?
It gave basic defintions of column names without considering that some columns might need more intepreations (e.g., Snow_Emerg, it is for snow storm name or street name?)
- How clear or explainable was the code it produced?
The code itself doesn't have enough comments to explain what each line does. The code itself seems clean though. 
...
It was pretty specific on the initial query, didn't need to ask it too many follow up questions. When we asked for more descriptive comments, it gave detailed comments on what the code did. 
- What would you have done differently if you wrote the code yourself?
We would have focused more on the aestetics of the visuals, codex mainy focused on just showing the data.
- Looking at the code Codex wrote, what could be improved (style, clarity, correctness, efficiency)?
It randomly flipped the coordinates when the x and y could have initially just been flipped. We wish it showed more of a summary as well. Codex also assumes that the data is already cleaned. It didn't call the variable 'Neighborho' correctly; it assumed it was called 'Neighborhood'.
...
- Ask Codex to help you map your data. Pay attention to what happens at first:
  ...
  It did correctly identify that it should plot points (each of which represent a towing). 
  - Does it confuse attributes (like IDs) with coordinates?
  It did know to use Latitude and Longitude but had written instructions to change column titles from Latitude and Longitude to whatever is in the data - even though it has the data and the columns were called Latitude and Longitude. 
...
We needed to identify what was missing from the original graph (Minneapolis boundary and roads) and explicitly tell Codex to include them. Adding this was easier with Codex because we didn't have to figure out what tigris functions we needed exactly. 
- Where did Codex help, and where did it get in the way?
It helped with the coding; from what we can see there were no mistakes in the code. It got in the way because it repeated what it already did, even though the task made it clear that it should build. The aesthetics are still not quite what we would want for a map; the points are tiny, it has slightly weird axis labels, and we don't really need the latitude and longitude displayed on the graph. 
- What would you do differently next time you had to map a new dataset with AI assistance?
Be more explicit from the beginning that there should be geographic context included on the map, instead of just having points. 
